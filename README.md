...
- `1枚比較処理`
  - `comparison` フォルダへ 6 パターンをまとめて出力
  - `x2 / sharp none`
  - `x2 / sharp weak`
  - `x2 / sharp medium`
  - `x4 / sharp none`
...
- 方式は軽い `Unsharp Mask` ベースです
- 強すぎるエッジ強調は避け、目元・口元・髪の細部が少し見やすくなる程度を狙っています
...
1. `比較用1枚` に画像を指定
2. `1枚比較処理` を実行
3. `comparison` フォルダの 6 パターンを見比べる

**大量処理前に必ず「テスト処理」か「1枚比較処理」で確認してください。**

最初の確認条件としては、まず **`2x + シャープ弱`** を推奨します。  
...
- シャープ処理ありの場合は `_sharp_weak` などを付けます
